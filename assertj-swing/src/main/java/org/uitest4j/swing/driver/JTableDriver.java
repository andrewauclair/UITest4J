...
import static org.uitest4j.swing.driver.JTableContentsQuery.tableContents;
import static org.uitest4j.swing.driver.JTableHasSelectionQuery.hasSelection;
...
public class JTableDriver extends JComponentDriver {
	...
	@RunsInEDT
	@Nullable
	private static String cellValue(final @Nonnull JTable table, final @Nonnull TableCell cell,
	...
	@RunsInEDT
	@Nullable
	private static String cellValue(final @Nonnull JTable table, final int row, final int column,
									final @Nonnull JTableCellReader cellReader) {
		return execute(() -> {
	...
	@RunsInEDT
	@Nullable
	private static Font cellFont(final @Nonnull JTable table, final @Nonnull TableCell cell,
								 final @Nonnull JTableCellReader cellReader) {
		return execute(() -> {
			JTableCellPreconditions.checkCellIndicesInBounds(table, cell);
	...
	@RunsInEDT
	@Nullable
	private static Color cellBackground(final @Nonnull JTable table, final @Nonnull TableCell cell,
										final @Nonnull JTableCellReader cellReader) {
		return execute(() -> {
			JTableCellPreconditions.checkCellIndicesInBounds(table, cell);
	...
	@RunsInEDT
	@Nullable
	private static Color cellForeground(final @Nonnull JTable table, final @Nonnull TableCell cell,
										final @Nonnull JTableCellReader cellReader) {
		return execute(() -> {
			JTableCellPreconditions.checkCellIndicesInBounds(table, cell);
	...
	public void selectCells(final @Nonnull JTable table, final @Nonnull TableCell[] cells) {
		...
		if (cells.length == 0) {
			throw new IllegalArgumentException("cells should be non-empty.");
		}
	...
	@RunsInEDT
	@Nonnull
	private Point scrollToPointAtCell(final @Nonnull JTable table, final @Nonnull TableCell cell,
	...
	@RunsInEDT
	@Nonnull
	private static Point pointAtCell(final @Nonnull JTable table, final @Nonnull TableCell cell,
									 final @Nonnull JTableLocation location) {
		Point result = execute(() -> {
			JTableCellPreconditions.checkCellIndicesInBounds(table, cell);
			return location.pointAt(table, cell.row, cell.column);
...
//		verifyThat(value(table, cell)).as(cellValueProperty(table, cell)).isEqualOrMatches(value);
	...
	private static void failColumnIndexNotFound(@Nonnull Object columnId) {
	...
	@RunsInEDT
	@Nonnull
	private Pair<Boolean, Point> cellSelectionInfo(final @Nonnull JTable table, final int row,
												   final int column,
												   final @Nonnull JTableLocation location) {
