...
public abstract class AbstractComponentFixture<S, C extends Component, D extends ComponentDriver> implements
	 ...
	 */
	public AbstractComponentFixture(@Nonnull Class<S> selfType, @Nonnull SwingRobot robot, @Nonnull Class<? extends C> type) {
		this(selfType, robot, findTarget(robot, type));
	}

	@Nonnull
	private static <C extends Component> C findTarget(@Nonnull SwingRobot robot, @Nonnull Class<? extends C> type) {
	 ...
	 * @param selfType the "self type."
	 * @param robot    performs simulation of user events on a {@code Component}.
	 * @param name     the name of the {@code Component} to find using the given {@code SwingRobot}.
	 * @param type     the type of the {@code Component} to find using the given {@code SwingRobot}.
	 * @throws NullPointerException                                  if {@code robot} is {@code null}.
	 * @throws NullPointerException                                  if {@code type} is {@code null}.
	 * @throws org.uitest4j.swing.exception.ComponentLookupException if a matching component could not be found.
	 * @throws org.uitest4j.swing.exception.ComponentLookupException if more than one matching component is found.
	 */
	public AbstractComponentFixture(@Nonnull Class<S> selfType, @Nonnull SwingRobot robot, @Nullable String name,
									@Nonnull Class<? extends C> type) {
		this(selfType, robot, findTarget(robot, name, type));
	}

	@Nonnull
	private static <C extends Component> C findTarget(@Nonnull SwingRobot robot, @Nullable String name,
													  @Nonnull Class<? extends C> type) {
		Objects.requireNonNull(robot);
		Objects.requireNonNull(type);
	 ...
	 */
	public AbstractComponentFixture(@Nonnull Class<S> selfType, @Nonnull SwingRobot robot, @Nonnull C target) {
	 ...
	 */
	public final @Nonnull
	S pressAndReleaseKey(@Nonnull KeyPressInfo keyPressInfo) {
	 ...
	 */
	public final @Nonnull
	S pressAndReleaseKeys(@Nonnull int... keyCodes) {
	 ...
	 */
	public final @Nonnull
	S pressKeyWhileRunning(int keyCode, @Nonnull Runnable runnable) {
	 ...
	 */
	public final @Nonnull
	S requireEnabled(@Nonnull Timeout timeout) {
	...
	<T extends C> T targetCastedTo(@Nonnull Class<T> type) {
		if (!target.getClass().isAssignableFrom(type)) {
			throw new AssertionFailedError("Cannot cast " + target.getClass() + " to " + type);
		}
