...
class JComboBoxDriver_requireSelectionByIndex_Test extends JComboBoxDriver_TestCase {
	...
	void should_Fail_If_JComboBox_Does_Not_Have_Expected_Selection() {
		selectFirstItem();
		ExpectedException.assertOpenTest4jError(() -> driver.requireSelection(comboBox, 1), "Expected selection of 'TestComboBox' to be '1' but was '0'", 1, 0);
	}

	@Test
	void should_Fail_If_JComboBox_Does_Not_Have_Any_Selection() {
		clearSelection();
		ExpectedException.assertOpenTest4jError(() -> driver.requireSelection(comboBox, 1), "Expected selection of 'TestComboBox' to be '1' but had no selection");
	}
}
