...
class TextAssert_matches_Test {
	...
	void should_Fail_If_Actual_Does_Not_Match_Regex_Pattern() {
		ExpectedException.assertContainsMessage(AssertionError.class, () -> new TextAssert("hello").matches(Pattern.compile("bye")), "Expecting: hello to match pattern: bye");
	}

	@Test
	void should_Fail_Showing_Description_If_Actual_Does_Not_Match_Regex_Pattern() {
		ExpectedException.assertContainsMessage(AssertionError.class, () -> new TextAssert("hello").as("A Test").matches(Pattern.compile("bye")), "[A Test] Expecting: hello to match pattern: bye");
	}
