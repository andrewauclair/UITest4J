...
class JListDriver_requireSelectionByIndex_Test extends JListDriver_TestCase {
	...
	void should_Fail_If_There_Is_No_Selection() {
		clearSelection();
		ExpectedException.assertOpenTest4jError(() -> driver.requireSelection(list, 0), "Expected selected index of 'TestList' to be '0' but had no selection");
	}

	@Test
	void should_Fail_If_Selection_Is_Not_Equal_To_Expected() {
		select(1);
		ExpectedException.assertOpenTest4jError(() -> driver.requireSelection(list, 0), "Expected selected index of 'TestList' to be '0' but was '1'", 0, 1);
	}
}
